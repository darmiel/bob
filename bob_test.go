...
func TestAny(t *testing.T) {
	...
	assert.Equal(t, "{Name:Bob Age:18} -- Alice is 18", b.String(), "WriteAny")
}

func TestAnyAll(t *testing.T) {
	var b *Builder
	b = new(Builder)
	b.WriteAll(byte('H'), []byte("el"), "lo", '!', 1, nil, true)
	...
	b.Reset()
	b.WriteAll("Hello", " World")
	assert.Equal(t, "Hello World", b.String(), "WriteAll")

	b.WriteAll(WriteWithNewLine(), " Hi!")
	assert.Equal(t, "Hello World Hi!\n", b.String(), "WriteAll")
	b.Reset()

	b = New(WithSeparator(" "), WithNewLine(), WithLineSeparator("\n\n"))
	b.WriteAll("Hello", "World", "What's", "up?")
